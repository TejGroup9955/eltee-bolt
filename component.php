...
if($Flag=="LoadPort")
{
    ...
    $rstdes = mysqli_query($connect,"select * from port_master where status='Active'");
    while($rwdes = mysqli_fetch_assoc($rstdes))
    {
...
if($Flag=="loadIncoterms")
{
    echo "<option value=''>Select Incoterms</option>";
    $rstdes = mysqli_query($connect,"select * from incoterms_master where status=1");
    while($rwdes = mysqli_fetch_assoc($rstdes))
    {
        $incoterms_id = $rwdes['incoterms_id'];
        $incoterms = $rwdes['incoterms'];
        $incoterms_fullform = $rwdes['incoterms_fullform'];
        echo "<option value='$incoterms_id'>$incoterms"." - "."$incoterms_fullform</option>";
    }
}
if($Flag=="loadModeOfShipment")
{
    ...
    while($rwdes = mysqli_fetch_assoc($rstdes))
    {
        ...
        $incoterms = $rwdes['incoterms'];
        $incoterms_fullform = $rwdes['incoterms_fullform'];
        echo "<option value='$incoterms'>$incoterms"." - "."$incoterms_fullform</option>";
    }
}
if($Flag=="LoadCountryDetails")
{
    $port_id = $_POST['port_id'];
    $rstport = mysqli_query($connect,"select p.*,cc.countryName from port_master p 
        inner join country_master cc on cc.id=p.country_id
        where p.port_master_id ='$port_id'");
    $rwport = mysqli_fetch_assoc($rstport);
    echo json_encode($rwport);
}
if($Flag=="LoadState")
{
...
if($Flag=="DeleteClient")
{
    $sql = "UPDATE client_master SET client_status='Delete' WHERE client_id = $client_id ";
    $connect->query($sql);
    $sql = "UPDATE client_requirement_details SET client_status='Delete' WHERE client_id = $client_id ";
    $connect->query($sql);
    $today_date=date("Y-m-d");
    $sql1 = "INSERT INTO client_delete_master (client_id,user_id,delete_date,delete_reason) VALUES('$client_id','$user_id_session','$today_date','$remark') ";
