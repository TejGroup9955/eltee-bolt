...
$rstpro = mysqli_query($connect,"select p.*,c.client_name,c.client_add,
c.client_email,c.client_mob,c.GST_no,
c.kind_attention,
co.countryName as CountryOfOrigin,cs.countryName as CountryOfShipment,cc.countryName as CurrencyCountry,
ccs.currency_code,
bd.bank_id,bd.branch,bd.bank_address,bd.po_box,bd.account_name,bd.account_no,bd.iban_no,bd.swift_code,pt.port_name,i.incoterms,
i.incoterms_fullform,py.pay_percentage,py.after_percentage,py.payment_desc_id,py.after_payment_desc_id
    ...
    $html.='
        <table style="width: 100%; border-collapse: collapse;">
            <tr>
                ...
                <th class="no-border" style="text-align: right; vertical-align: middle; width: 70%;">
                    <h2 style="margin: 0;">Proforma Invoice & Sales Contract</h2><br><br>
                    <p style="margin: 5px 0;"><b>PI Number: '.$rwpro['pi_custom_number'].'<br>
                    Date: '.date('d M Y',strtotime($rwpro['pi_invoice_date'])).'</b></p>
                </th>
            </tr>
        </table>
        <table style="width: 100%; border-collapse: collapse;">
            <tr>
                ...
                <td style="width: 50%;">
                      <strong>BUYER: '.$rwpro['client_name'].'</strong><br> 
                        ...
                        GST : '.$rwpro['GST_no'].'<br>
                        Email ID : '.$rwpro['client_email'].'<br>
                        Contact No : '.$rwpro['client_mob'].'<br>
        ...
        <table>
            <tr style="background-color:#1a7af1;">
                ...
                <th style="color:white;" class="center">NO. OF PACKAGE</th>
...
where d.pi_no='$Pi_No'");
        if(mysqli_num_rows($rstprod)>0)
        {
            ...
            while($rwprod = mysqli_fetch_assoc($rstprod))
            {
                ...
                $html.='<tr>
                        ...
                        <td class="center">'.$rwprod['each_bag_weight'].' kg '.$rwprod['packaging_type'].' </td>
		...
		//$total_in_word = numberToWords($grand_total);
        ...
        $html.='</table>  
        <p>
         <h4 >Remark:  </h4> '.$rwpro['remark'].'</p><br>
        <br>
        <h3>OUR BANKERS </h3>
