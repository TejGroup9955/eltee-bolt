...
$account_id = "";
$cmd ="select p.*,c.client_name,u.user_name from purchase_order p 
...
if ($result->num_rows > 0) {
	if($row = $result->fetch_assoc()) {
		...
		$supplier_pi_date = $row['supplier_pi_date'];
		$po_date = $row['po_date'];
		...
		if($row["supplier_pi_date"] == "0000-00-00"){
			$supplier_pi_date = "-";
		}else{
			$supplier_pi_date=date('d-m-Y', strtotime($row["supplier_pi_date"]));
		}
...
$cmd12 = "SELECT SUM(paid_amount) as paid_amt FROM purchase_order_receipt_payment WHERE po_no = '$po_id' AND approval_status <> 0";
...
<div class="col-md-12"><h4 style="font-size:16pt;text-align:center;font-weight:bold;">Supplier Payment Details </h4>
</div>

<div class="col-md-6">
<h4 style="font-size: 11pt;"><b>Supplier Name : </b>'.$account_name.'</h4>
<h4 style="font-size: 11pt;"><b>Supplier PI Number :  </b>'.$supplier_pi_no.'</h4>
<h4 style="font-size: 11pt;"><b>Supplier PI Date :  </b>'.$supplier_pi_date.'</h4>
<h4 style="font-size: 11pt;"><b>PO No :  </b>'.$po_custom_number.'</h4>
</div>
<div class="col-md-6">
<h4 style="font-size: 11pt;text-align:right;"><b>PO Valid Upto : </b>'.$valid_upto.'</h4>
<h4 style="font-size: 11pt;text-align:right;"><b>Total Amount : </b>'.$grand_total.'</h4>
<h4 style="font-size: 11pt;text-align:right;"><b>PO Payment Status : &nbsp; </b>';
...
<h4 style="font-size: 11pt;text-align:right;"><b>PO Date :  </b>'.$po_date.'</h4>
</div>
<table>
<tr>
...
$cmd4 = "SELECT pr.total_amount, pr.remain_amount, pr.paid_amount, pr.paid_date, p.payment_mode, p.payment_method_id FROM purchase_order_receipt_payment pr INNER JOIN customer_information p ON p.payment_method_id = pr.payment_mode WHERE pr.po_no=".$po_id." AND pr.approval_status <> 0 ORDER BY pr.customer_receipt_id ";
